...
#+header: :results output file graphics
#+header: :file Kayal_Figure02_pca-struct-relate.new.pdf
#+header: :bg white :width 8.5 :height 11
#+begin_src R :exports both
grid.newpage()
pushViewport(viewport(
  layout = grid.layout(nrow = 3, ncol = 2,
...
#+header: :results output file graphics
#+header: :file Kayal_Figure02_pca-struct-relate.newHV.pdf
#+header: :bg white :width 8.5 :height 11
#+begin_src R :exports both
grid.newpage()
pushViewport(viewport(
...
c(1,1,2,2),
c(1,3,1,3),
eigen.plots[c("All", "KcHV", "Kd", "Kk")])
popViewport()
popViewport()


num.per.species <-
  structure.data[c('KcHV','Kd','Kk')] %>%
  lapply(pull, individual) %>%
  lapply(unique) %>%
  sapply(length) %>%
  scale(center = F) *2
...
popViewport()
       ...
       rep(c(2,4,6), 2),
       c(plot.interspecific[c("Kc", "Kd", "Kk")],
         plot.intraspecific[c("KcHV", "Kd", "Kk")]))
