...
other attached packages:
 [1] jpeg_0.1-10             rnaturalearthdata_1.0.0 rnaturalearth_1.0.1     sf_1.0-18               lubridate_1.9.2        
 [6] forcats_1.0.0           stringr_1.5.1           dplyr_1.1.4             purrr_1.0.2             readr_2.1.4            
[11] tidyr_1.3.1             tibble_3.2.1            ggplot2_3.5.1           tidyverse_2.0.0        

loaded via a namespace (and not attached):
 [1] Rcpp_1.0.11        pillar_1.9.0       compiler_4.2.2     class_7.3-22       tools_4.2.2        jsonlite_1.8.7    
 [7] lifecycle_1.0.4    gtable_0.3.5       timechange_0.2.0   pkgconfig_2.0.3    rlang_1.1.3        cli_3.6.2         
[13] DBI_1.2.2          terra_1.7-83       e1071_1.7-16       httr_1.4.7         withr_3.0.0        generics_0.1.3    
[19] vctrs_0.6.5        hms_1.1.3          classInt_0.4-10    tidyselect_1.2.1   glue_1.7.0         R6_2.5.1          
[25] fansi_1.0.6        tzdb_0.4.0         magrittr_2.0.3     codetools_0.2-19   scales_1.3.0       units_0.8-5       
[31] colorspace_2.1-0   utf8_1.2.4         KernSmooth_2.23-24 stringi_1.8.4      proxy_0.4-27       munsell_0.5.1
#+end_example
...
#+begin_src R :exports both
havo.loc <-
  data.frame(lat=19.506785398861332, lon=-155.4641966439659)
...
hawaii.plot <- ggplot(data = world) +
  ...
  scale_fill_manual(values=species.color,
                    ...
                    labels=c("Kc" = "K. cookei",
                             "Kd" = "K. drynarioids",
                             "Kk" = "K. kauaiensis")) +
  annotate(geom = "point", x = havo.loc$lon, y = havo.loc$lat,
           shape=21, size=2, color="grey90",
           fill=species.color["Kc"]) +
  annotate(geom = "text", x = havo.loc$lon, y = havo.loc$lat,
           label = "HAVO",
##           fontface = "italic",
           color = "grey90",
           vjust = -0.8,
...
grid.lines(x = unit(c(0.80, 0.315), "npc"),
          y = unit(c(0.98, 0.79), "npc"),
          default.units = "npc",
          arrow(length = unit(3, "mm")),
          gp=gpar(col='grey50'))
grid.lines(x = unit(c(0.80, 0.525), "npc"),
          y = unit(c(0.66, 0.63), "npc"),
          default.units = "npc",
          arrow(length = unit(3, "mm")),
          gp=gpar(col='grey50'))
grid.lines(x = unit(c(0.80, 0.68), "npc"),
          y = unit(c(0.35, 0.355), "npc"),
          default.units = "npc",
          arrow(length = unit(3, "mm")),
          gp=gpar(col='grey50'))
...
#swap Kd_HV_1Ar and Kd_HV_1Br
sample.order[3] <- "Kd_HV_1Br"
sample.order[4] <- "Kd_HV_1Ar"
...
mapply(write.table,
       ...
       interspecific.data,
       sprintf("inter.%s.lea.txt", names(interspecific.data)))

mapply(write.table,
       MoreArgs = list( sep="\t", quote=F, row.names=F),
       intraspecific.data,
       sprintf("intra.%s.lea.txt", names(intraspecific.data)))
#+end_src
...
*** COMMENT Separate Kd structure by Location
...
list("inter" = bind_rows(interspecific.data, .id='Species'),
  ...
  bind_rows(.id="Compare") %>%
  filter(Species != "Kc" | Compare == "inter") %>%
  mutate(color = ifelse(Compare=="intra", Species, "inter"),
         color = paste(color, pop, sep='.')) %>%
  mutate(facet.col = factor(Compare, c("inter", "intra"),
                            c("Interspecific", "Intraspecific")),
         facet.row= factor(Species, c("KcHV", "Kc", "Kd", "Kk"),
                           c("K. cookei","K. cookei",
                                      "K. drynarioids",
                                      "K. kauaiensis"))) %>%
  ggplot(aes(q, order, fill=color)) +
...
#+RESULTS:
[[file:Kayal_SuppFigXX_sturcture.pdf]]
